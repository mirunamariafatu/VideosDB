...
/**
 * Class that contains methods of processing user data, and general information
 * about an user, retrieved from input
 */
public class User {
    /**
     * User's username
    ...
    public final String videoView(final ActionInputData command) {
        ...
        if (isInHistory(command.getTitle())) {
            ...
            message = "success -> " + command.getTitle() + " was viewed with total views of "
                    + newValue;
        } else {
    ...
    public final String serialRating(final Show serial, final ActionInputData command) {
        ...
        List<String> usersWhoRated = ((Serial) serial).getSeasons()
                .get(command.getSeasonNumber() - 1).getUsersRating();
        List<Double> ratingList = ((Serial) serial).getSeasons()
                .get(command.getSeasonNumber() - 1).getRatings();

        if (usersWhoRated != null) {
            for (int i = 0; i < usersWhoRated.size(); i++) {
                String tempUsername = ((Serial) serial).getSeasons()
                        .get(command.getSeasonNumber() - 1).getUsersRating().get(i);
        ...
        if (foundName == 0) {
            ...
            message = "success -> " + command.getTitle() + " was rated with " + command.getGrade()
    ...
    /**
     * This method finds the first video from database not seen by the user and
     * proposes it for recommendation
     *
     * @param videoData information about videos'database
     * @return the final message to be displayed
     */
    public final String getStandardVideo(final VideoDataBase videoData) {
    ...
    /**
     * This method searches for the first video with the highest rating, but not
     * seen by the user
     *
     * @param videoData information about videos'database
     * @param command   current action
     * @return
     */
    public final String getBestUnseenVideo(final VideoDataBase videoData,
            final ActionInputData command) {
        String message = "BestRatedUnseenRecommendation result: ";

        // Get the sorted HashMap based on the videos'ratings
        ...
        // Iterate through the ratings'map
